...
def fetch_images_and_videos(search_query):
    ...
    for item in items:
        ...
        if media_type == "image":
            ...
            preview_link = links[0].get("href")
            # print(preview_link)
            if preview_link not in seen_video_ids:
                api_media['videos'].append({'nasa_id': nasa_id, 'preview_link': preview_link})
                seen_video_ids.add(preview_link)             
    return api_media
...
def index():
    ...
    if request.method == 'POST':
        ...
        return render_template('base.html', modal_resources=modal_resources, api_media=api_media, videos_manifests=videos_manifests, search_query=search_query)
